...
const loadRuleData = async () => {
  ...
  try {
    const response = await api.get(`/api/v1/inspection-tasks/${ruleId}`)
    const rule = response.data
    
    // 检查是否为自定义Cron
    const isCustomCron = !cronOptions.some(option => option.value === rule.cron_schedule && option.value !== 'custom')
    ...
    ruleForm.value = {
      ...
      expected_sql: rule.expected_sql,
    ...
    if (isCustomCron) {
      ...
      customCron.value = rule.cron_schedule
    }
  } catch {
...
const submitRule = async () => {
  ...
  loading.value = true
  try {
    await api.put(`/api/v1/inspection-tasks/${ruleId}`, ruleForm.value)
    ElMessage.success('巡检规则更新成功')
    router.push('/inspection-rules')
  } catch {
    ElMessage.error('更新巡检规则失败')
  } finally {
    loading.value = false
  }
}
      ...
      <div v-else class="form-content">
        ...
        <div class="form-section">
          ...
          <div class="rule-description">
            ...
            <div class="description-item">
              <h4>📊 监控指标</h4>
              <p>规则更新后，您可以在巡检中心查看执行历史、成功率和统计信息。</p>
            </div>
          </div>
        </div>
      </div>
